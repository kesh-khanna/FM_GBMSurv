...
def set_trainable_uniformer(
    ...
    # patch_embed1 (raw input stem)
    if train_patch_embed1 or train_stage1:
        count = sum(p.numel() for p in model.patch_embed1.parameters())
        trainable_params += count
        for p in model.patch_embed1.parameters():
            p.requires_grad = True
        if debug:
            print(f"patch_embed1: {count:,} parameters")

    # stage1 blocks
    if train_stage1:
        count = sum(p.numel() for p in model.blocks1.parameters())
        trainable_params += count
        for p in model.blocks1.parameters():
            p.requires_grad = True
        if debug:
            print(f"blocks1: {count:,} parameters")

    # stage2 stem + blocks
    if train_stage2:
        count_embed = sum(p.numel() for p in model.patch_embed2.parameters())
        count_blocks = sum(p.numel() for p in model.blocks2.parameters())
        trainable_params += count_embed + count_blocks
        for p in model.patch_embed2.parameters():
            p.requires_grad = True
        for p in model.blocks2.parameters():
            p.requires_grad = True
        if debug:
            print(f"patch_embed2: {count_embed:,} parameters")
            print(f"blocks2: {count_blocks:,} parameters")

    # stage3 stem + blocks
    if train_stage3:
        count_embed = sum(p.numel() for p in model.patch_embed3.parameters())
        count_blocks = sum(p.numel() for p in model.blocks3.parameters())
        trainable_params += count_embed + count_blocks
        for p in model.patch_embed3.parameters():
            p.requires_grad = True
        for p in model.blocks3.parameters():
            p.requires_grad = True
        if debug:
            print(f"patch_embed3: {count_embed:,} parameters")
            print(f"blocks3: {count_blocks:,} parameters")

    # stage4 stem + blocks
    if train_stage4:
        count_embed = sum(p.numel() for p in model.patch_embed4.parameters())
        count_blocks = sum(p.numel() for p in model.blocks4.parameters())
        trainable_params += count_embed + count_blocks
        for p in model.patch_embed4.parameters():
            p.requires_grad = True
        for p in model.blocks4.parameters():
            p.requires_grad = True
        if debug:
            print(f"patch_embed4: {count_embed:,} parameters")
            print(f"blocks4: {count_blocks:,} parameters")

    # final BN(512)
    if train_final_norm:
        count = sum(p.numel() for p in model.norm.parameters())
        trainable_params += count
        for p in model.norm.parameters():
            p.requires_grad = True
        if debug:
            print(f"norm: {count:,} parameters")

    # optional: train all LN / BN 
    if train_all_layernorm:
        ...
        if debug and count > 0:
            print(f"LayerNorm (additional): {count:,} parameters")

    if train_all_batchnorm:
        count = 0
        for m in model.modules():
        ...
        trainable_params += count
        if debug and count > 0:
            print(f"BatchNorm3d (additional): {count:,} parameters")
